<script lang="ts">
	...
	import type { AntiparasiticAnalyticsAntiparasitic, AntiparasiticTreatmentDueFilterMode, AntiparasiticTreatmentHistoryPeriod, AntiparasiticTreatmentStatusItem, AntiparasiticTreatmentStatusKey, AntiparasiticTreatmentStatusSummary } from '$lib/domain/antiparasitic/analytics.js';
	import { emptyAntiparasiticTreatmentStatusSummary, shiftIsoDate, todayIsoDate, antiparasiticTreatmentHistoryPeriods, antiparasiticTreatmentStatusKeys } from '$lib/domain/antiparasitic/analytics.js';
	import { formatDateForDisplay, formatDateForInput } from '$lib/domain/shared/date-input.js';
	import {
		loadAnalyticsAntiparasitics,
		loadAntiparasiticTreatmentAnalyticsOverview,
		loadAntiparasiticTreatmentHistory,
		loadAntiparasiticTreatmentStatusItems
	} from '$lib/services/antiparasitic-analytics.service.js';
	...
	const { basePath = '/antiparasitics' } = $props<{ basePath?: string }>();

	const statusOptions: { status: AntiparasiticTreatmentStatusKey; labelKey: TranslationKey; detailKey: TranslationKey; barClass: string; textClass: string }[] = [
		{ status: 'current', labelKey: 'antiparasiticTreatment.status.current', detailKey: 'antiparasiticTreatment.status.currentDetail', barClass: 'bg-emerald-600', textClass: 'text-emerald-700' },
		{ status: 'dueSoon', labelKey: 'antiparasiticTreatment.status.dueSoon', detailKey: 'antiparasiticTreatment.status.dueSoonDetail', barClass: 'bg-sky-600', textClass: 'text-sky-700' },
		{ status: 'dueVerySoon', labelKey: 'antiparasiticTreatment.status.dueVerySoon', detailKey: 'antiparasiticTreatment.status.dueVerySoonDetail', barClass: 'bg-amber-600', textClass: 'text-amber-700' },
		{ status: 'expired', labelKey: 'antiparasiticTreatment.status.expired', detailKey: 'antiparasiticTreatment.status.expiredDetail', barClass: 'bg-orange-600', textClass: 'text-orange-700' },
		{ status: 'overdue', labelKey: 'antiparasiticTreatment.status.overdue', detailKey: 'antiparasiticTreatment.status.overdueDetail', barClass: 'bg-destructive', textClass: 'text-destructive' }
	];
	...
	let dueFilterMode = $state<AntiparasiticTreatmentDueFilterMode>('status');
	let status = $state<AntiparasiticTreatmentStatusKey>('expired');
	let periodStartDate = $state(defaultPeriodStartDate);
	let periodEndDate = $state(defaultPeriodEndDate);
	let period = $state<AntiparasiticTreatmentHistoryPeriod>('month');
	let antiparasiticNormalizedName = $state('');
	let activeTab = $state<ActiveTab>('status');
	let statusOrder = $state<SortOrder>('recent');
	let historyOrder = $state<SortOrder>('recent');
	let items = $state<AntiparasiticTreatmentStatusItem[]>([]);
	let visibleItems = $state<AntiparasiticTreatmentStatusItem[]>([]);
	let avatarBytesByPetId = $state(new Map<number, Uint8Array | null>());
	let statusSummary = $state<AntiparasiticTreatmentStatusSummary>(emptyAntiparasiticTreatmentStatusSummary());
	let statusTotalTracked = $state(0);
	let history = $state<HistoryPoint[]>([]);
	let analyticsAntiparasitics = $state<AntiparasiticAnalyticsAntiparasitic[]>([]);
	let statusLoading = $state(false);
	let statusListLoading = $state(false);
	let historyLoading = $state(false);
	let antiparasiticsLoading = $state(false);
	let statusLoaded = $state(false);
	let historyLoaded = $state(false);
	let antiparasiticsLoaded = $state(false);
	...
	function normalizeStatus(value: string | null): AntiparasiticTreatmentStatusKey {
	...
	function normalizeDueFilterMode(value: string | null): AntiparasiticTreatmentDueFilterMode {
	...
	function normalizePeriod(value: string | null): AntiparasiticTreatmentHistoryPeriod {
	...
	function periodLabelKey(value: AntiparasiticTreatmentHistoryPeriod): TranslationKey {
		return `antiparasiticTreatment.history.period.${value}` as TranslationKey;
	}

	function statusLabelKey(value: AntiparasiticTreatmentStatusKey): TranslationKey {
		return `antiparasiticTreatment.status.${value}` as TranslationKey;
	}

	function statusSectionTitleKey(): TranslationKey {
		return dueFilterMode === 'status' ? statusLabelKey(status) : 'antiparasiticTreatment.analytics.periodFilterTitle';
	}

	function statusCount(value: AntiparasiticTreatmentStatusKey): number {
		return statusSummary[value] ?? 0;
	}

	function statusPercent(value: AntiparasiticTreatmentStatusKey): number {
	...
	function statusPercentLabel(value: AntiparasiticTreatmentStatusKey): string {
		const count = statusCount(value);
	...
	function sortStatusItems(source: AntiparasiticTreatmentStatusItem[], order: SortOrder): AntiparasiticTreatmentStatusItem[] {
		...
		return [...source].sort((first, second) => {
			...
			return ownerDisplayName(first).localeCompare(ownerDisplayName(second)) || first.petName.localeCompare(second.petName) || first.antiparasiticName.localeCompare(second.antiparasiticName);
		});
	}

	function ownerDisplayName(item: AntiparasiticTreatmentStatusItem): string {
		return item.ownerName || t('owner.unassigned');
	}

	function petProfileHref(item: AntiparasiticTreatmentStatusItem): string {
		return `/pets/${item.petId}`;
	}

	function petAvatarBytes(item: AntiparasiticTreatmentStatusItem): Uint8Array | null {
	...
	function updateUrl() {
		...
		if (antiparasiticNormalizedName) params.set('antiparasitic', antiparasiticNormalizedName);
		window.history.replaceState(null, '', `${basePath}?${params.toString()}`);
	}

	function isRefreshing(): boolean {
		return activeTab === 'status' ? statusLoading : historyLoading || antiparasiticsLoading;
	}
	...
	async function renderStatusItemsInChunks(source: AntiparasiticTreatmentStatusItem[], order = statusOrder) {
	...
	async function loadVisiblePetAvatars(source: AntiparasiticTreatmentStatusItem[]): Promise<void> {
	...
	async function loadStatusData() {
		...
		try {
			const [loadedItems, loadedOverview] = await Promise.all([
				loadAntiparasiticTreatmentStatusItems({ mode: requestedFilterMode, status: requestedStatus, startDate: requestedStartDate, endDate: requestedEndDate }),
				loadAntiparasiticTreatmentAnalyticsOverview()
			]);
	...
	async function loadHistoryData() {
		...
		const requestedAntiparasitic = antiparasiticNormalizedName;
		historyLoading = true;
		historyError = '';
		try {
			const loadedHistory = await loadAntiparasiticTreatmentHistory({ period: requestedPeriod, antiparasiticNormalizedName: requestedAntiparasitic || null });
			if (requestId !== historyRequestId || requestedPeriod !== period || requestedAntiparasitic !== antiparasiticNormalizedName) return;
	...
	async function loadAntiparasiticData(force = false) {
		if (!force && antiparasiticsLoaded) {
			antiparasiticsLoading = false;
			return;
		}

		const requestId = ++antiparasiticsRequestId;
		antiparasiticsLoading = true;
		historyError = '';
		try {
			const loadedAntiparasitics = await loadAnalyticsAntiparasitics();
			if (requestId !== antiparasiticsRequestId) return;
			analyticsAntiparasitics = loadedAntiparasitics;
			antiparasiticsLoaded = true;
		} catch (err) {
			if (requestId !== antiparasiticsRequestId) return;
			console.error(err);
			historyError = err instanceof Error ? err.message : t('common.error');
		} finally {
			if (requestId === antiparasiticsRequestId) antiparasiticsLoading = false;
		}
	}
	...
	function queueHistoryLoad(forceAntiparasitics = false) {
		historyLoading = true;
		historyLoaded = false;
		if (forceAntiparasitics || !antiparasiticsLoaded) antiparasiticsLoading = true;
		void waitForNextPaint().then(() => Promise.all([loadHistoryData(), loadAntiparasiticData(forceAntiparasitics)]));
	}
	...
	function selectTab(tab: ActiveTab) {
		...
		if (tab === 'history' && ((!historyLoaded && !historyLoading) || (!antiparasiticsLoaded && !antiparasiticsLoading))) queueHistoryLoad();
	}
	...
	function selectPeriod(nextPeriod: AntiparasiticTreatmentHistoryPeriod) {
		activeTab = 'history';
		period = nextPeriod;
		updateUrl();
		queueHistoryLoad();
	...
	function selectAntiparasitic(value: string) {
		activeTab = 'history';
		antiparasiticNormalizedName = value;
		updateUrl();
		queueHistoryLoad();
	}
	...
	function daysText(item: AntiparasiticTreatmentStatusItem): string {
		const absoluteDays = Math.abs(item.daysUntilDue);
		const unit = absoluteDays === 1 ? t('antiparasiticTreatment.daySingular') : t('antiparasiticTreatment.dayPlural');
		if (item.daysUntilDue < 0) return `${absoluteDays} ${unit} ${t('antiparasiticTreatment.status.daysOverdue')}`;
		if (item.daysUntilDue === 0) return t('antiparasiticTreatment.status.expiresToday');
		return `${item.daysUntilDue} ${unit} ${t('antiparasiticTreatment.status.daysUntilDue')}`;
	}
	...
	function openContactDialog(item: AntiparasiticTreatmentStatusItem) {
	...
	function initialActiveTab(params: URLSearchParams): ActiveTab {
		...
		return params.has('period') || params.has('antiparasitic') ? 'history' : 'status';
	}
	...
	onMount(() => {
		if (typeof window !== 'undefined') {
			...
			antiparasiticNormalizedName = params.get('antiparasitic') ?? '';
...
<svelte:head>
	<title>{t('antiparasiticTreatment.analytics.title')} | {t('app.name')}</title>
</svelte:head>
...
<section class="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
	<header class="flex flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
		<div class="min-w-0">
			<p class="text-sm font-medium text-muted-foreground">{t('antiparasiticTreatment.analytics.kicker')}</p>
			<h2 class="text-2xl font-semibold tracking-normal text-foreground sm:text-3xl">{t('antiparasiticTreatment.analytics.title')}</h2>
			<p class="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{t('antiparasiticTreatment.analytics.description')}</p>
		</div>
	...
	<div class="grid grid-cols-2 gap-1 rounded-md border border-border bg-muted p-1" role="tablist" aria-label={t('antiparasiticTreatment.analytics.title')}>
		<button
		...
		>
			<Pill class="size-4" />
			<span class="truncate">{t('antiparasiticTreatment.analytics.statusTitle')}</span>
		</button>
		<button
		...
		>
			...
			<span class="truncate">{t('antiparasiticTreatment.analytics.historyTitle')}</span>
		</button>
	</div>
	...
	{#if activeTab === 'status'}
		<section class="min-w-0 rounded-md border border-border bg-card p-4 shadow-sm sm:p-5" role="tabpanel">
			<div class="flex items-center gap-2 text-sm font-medium text-muted-foreground">
				<Pill class="size-4" />
				{t('antiparasiticTreatment.analytics.statusTitle')}
			</div>

			<div class="mt-4 grid gap-3 rounded-md border border-border bg-background p-3 lg:grid-cols-[14rem_minmax(0,1fr)]">
				<div class="space-y-1">
					<label class="text-sm font-medium" for="due-filter-mode">{t('antiparasiticTreatment.analytics.filterMode')}</label>
					<Select
						id="due-filter-mode"
						value={dueFilterMode}
						options={[
							{ value: 'status', label: t('antiparasiticTreatment.analytics.filterMode.status') },
							{ value: 'period', label: t('antiparasiticTreatment.analytics.filterMode.period') }
						]}
				...
				{#if dueFilterMode === 'status'}
					<div class="space-y-2">
						<span class="text-sm font-medium">{t('antiparasiticTreatment.analytics.statusFilter')}</span>
				...
				{:else}
					<div class="grid gap-3 md:grid-cols-3">
						<div class="space-y-1">
							<span class="text-sm font-medium">{t('antiparasiticTreatment.analytics.periodStartDate')}</span>
							<DateField bind:value={periodStartDate} max={todayDate} ariaLabel={t('antiparasiticTreatment.analytics.periodStartDate')} onChange={selectPeriodStartDate} />
						</div>
						<div class="space-y-1">
							<span class="text-sm font-medium">{t('antiparasiticTreatment.analytics.periodToday')}</span>
						...
						<div class="space-y-1">
							<span class="text-sm font-medium">{t('antiparasiticTreatment.analytics.periodEndDate')}</span>
							<DateField bind:value={periodEndDate} min={todayDate} ariaLabel={t('antiparasiticTreatment.analytics.periodEndDate')} onChange={selectPeriodEndDate} />
						</div>
					</div>
				{/if}
			</div>
			...
			<div class="mt-5 border-t border-border pt-4">
				<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
					<div class="min-w-0">
						...
						<p class="mt-1 text-sm text-muted-foreground">{t('antiparasiticTreatment.analytics.listDescription')}</p>
					</div>
					<div class="flex flex-col gap-2 sm:items-end">
						<label class="text-sm font-medium" for="status-order">{t('antiparasiticTreatment.analytics.order')}</label>
						<div class="flex items-center gap-2">
							<Select
								...
								value={statusOrder}
								options={[
									{ value: 'recent', label: t('antiparasiticTreatment.analytics.order.recent') },
									{ value: 'old', label: t('antiparasiticTreatment.analytics.order.old') }
								]}
				...
				{:else if items.length === 0}
					<p class="mt-4 rounded-md border border-dashed border-border px-4 py-8 text-center text-sm text-muted-foreground">{t('antiparasiticTreatment.analytics.emptyStatus')}</p>
				{:else}
					<div class="mt-4 divide-y divide-border rounded-md border border-border">
						{#each visibleItems as item (`${item.petId}:${item.antiparasiticNormalizedName}`)}
							<article class="grid gap-3 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-4">
								<div class="flex min-w-0 items-start gap-3">
									...
									<div class="min-w-0">
										<p class="wrap-break-word text-sm font-semibold">{item.petName} · {item.antiparasiticName}</p>
										...
										<div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs leading-5 text-muted-foreground">
											<span>{t('antiparasiticTreatment.appliedAt')}: {formatDateForDisplay(item.appliedAt)}</span>
											<span>{t('antiparasiticTreatment.analytics.dueAt')}: {formatDateForDisplay(item.dueAt)}</span>
	...
	{:else}
		<section class="min-w-0 rounded-md border border-border bg-card p-4 shadow-sm sm:p-5" role="tabpanel">
			<div class="flex items-center gap-2 text-sm font-medium text-muted-foreground">
				<CalendarDays class="size-4" />
				{t('antiparasiticTreatment.analytics.historyTitle')}
			</div>
			...
			<div class="mt-4 grid gap-5 lg:grid-cols-[18rem_minmax(0,1fr)]">
				<div class="space-y-3">
					<label class="block text-sm font-medium" for="history-period">{t('antiparasiticTreatment.analytics.period')}</label>
					<div id="history-period" class="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2">
						{#each antiparasiticTreatmentHistoryPeriods as option}
						...
						<div class="space-y-1">
							<label class="block text-sm font-medium" for="history-antiparasitic">{t('antiparasiticTreatment.analytics.antiparasiticFilter')}</label>
							<Select
								id="history-antiparasitic"
								value={antiparasiticNormalizedName}
								disabled={antiparasiticsLoading || historyLoading}
								options={[
									{ value: '', label: t('antiparasiticTreatment.analytics.allAntiparasitics') },
									...analyticsAntiparasitics.map((antiparasitic) => ({ value: antiparasitic.antiparasiticNormalizedName, label: antiparasitic.antiparasiticName }))
								]}
								onchange={(value) => selectAntiparasitic(value as string)}
							/>
						</div>

						<div class="space-y-1">
							<label class="block text-sm font-medium" for="history-order">{t('antiparasiticTreatment.analytics.order')}</label>
							<Select
								id="history-order"
								value={historyOrder}
								options={[
						...
						<div class="min-w-0">
							{#if historyLoading || antiparasiticsLoading}
								<div class="space-y-3">
									{#each Array(8) as _}
							...
							{:else if sortedHistory.length === 0}
								<p class="rounded-md border border-dashed border-border px-4 py-8 text-center text-sm text-muted-foreground">{t('antiparasiticTreatment.analytics.emptyHistory')}</p>
							{:else}

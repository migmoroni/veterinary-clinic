...
export async function loadPetProfile(petId: number): Promise<PetProfile> {
	...
	const [owners, records, vaccinations, vaccines] = await Promise.all([listOwnersByPet(pet.id), listRecordsByPet(pet.id), listVaccinationsByPet(pet.id), listVaccines()]);

	return { pet, owners, records, vaccinations, vaccines };
}

...
pub(super) fn read(connection: &Connection) -> Result<SystemRows, String> {
    ...
    result.insert(
        ...
        query(connection, "SELECT id, place_type, parent_place_id, country_codes_json, latitude, longitude, name, normalized_name, aliases_json FROM geo_places ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT id, domain_id, kingdom_id, phylum_id, class_id, order_id, family_id, genus_id, species_id, breed_id, variety_id, size_term_key, name, normalized_name, aliases_json, stage_metrics_json, content_json FROM life_reference_items ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT life_id, place_id, sort_order FROM life_origin_places ORDER BY life_id, sort_order", |row| {
            Ok(SystemRow::LifeOrigin { life_id: row.get(0)?, place_id: row.get(1)?, sort_order: row.get(2)? })
        })?,
    );
    result.insert(
        SystemTable::ManufacturerCatalogItems,
        query(connection, "SELECT id, name, normalized_name, aliases_json, regions_json, website, content_json FROM manufacturer_catalog_items ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT id, name, normalized_name, aliases_json, regions_json, nomenclature_json, atc_vet_code, atc_vet_system, denominations_json, content_json FROM active_ingredient_catalog_items ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT id, name, normalized_name, aliases_json, regions_json, content_json FROM condition_catalog_items ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT id, name, normalized_name, applicable_taxon_ids_json, aliases_json, manufacturer_id, regions_json, regulatory_identifiers_json, commercial_line, presentation_dosage, target_species_warnings_json, content_json FROM product_catalog_items ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT entity_type, entity_id, taxonomy_id, term_key, sort_order FROM entity_taxonomy_terms ORDER BY entity_type, entity_id, taxonomy_id, sort_order", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT product_id, active_ingredient_id, sort_order FROM product_active_ingredients ORDER BY product_id, sort_order", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT id, kind, name, normalized_name, applicable_taxon_ids_json, observation FROM treatment_protocols ORDER BY id", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT protocol_id, product_id, sort_order FROM treatment_protocol_items ORDER BY protocol_id, sort_order", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT protocol_id, dose_id, label, validity_value, validity_unit, sort_order FROM treatment_protocol_doses ORDER BY protocol_id, sort_order", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT entity_type, entity_id, value, normalized_value, provenance, sort_order FROM entity_search_terms ORDER BY entity_type, entity_id, sort_order", |row| {
    ...
    result.insert(
        ...
        query(connection, "SELECT entity_type, entity_id, role, media_key, sort_order FROM entity_media_references ORDER BY entity_type, entity_id, role, sort_order", |row| {
...
pub(super) fn verify(observed: &SystemRows, contract: &ProjectionContract) -> Result<(), String> {
...
mod tests {
    ...
    fn fixed_writers_and_independent_readers_round_trip_every_system_row_case() {
        ...
        let observed = read(&connection).unwrap();

//! System font discovery command.
//!
//! The UI asks for a plain list of font family names. Platform probing and
//! optional extra directory scanning stay here instead of leaking into `lib.rs`.
...
#[tauri::command]
pub fn list_system_fonts(extra_directories: Option<Vec<String>>) -> Vec<String> {
...
fn extra_directory_fonts(directories: &[String]) -> Vec<String> {
    dedupe_and_sort(
        directories
            .iter()
            .flat_map(|directory| font_files_in_directory(Path::new(directory)))
            .filter_map(|path| {
                path.file_stem()
                    .and_then(|stem| stem.to_str())
                    .map(font_name_from_file_stem)
            })
            .filter_map(|name| normalize_font_name(&name)),
    )
}

fn font_files_in_directory(directory: &Path) -> Vec<PathBuf> {
    let mut files = Vec::new();
    collect_font_files(directory, &mut files, 0);
    files
}

fn collect_font_files(directory: &Path, files: &mut Vec<PathBuf>, depth: usize) {
    if depth > 4 {
        return;
    }

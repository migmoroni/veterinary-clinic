...
- Pet vaccination records use editable catalogs for vaccine names, dose types, and validity options. `vaccines` stores reusable names, `vaccine_dose_types` stores type labels such as initial dose or booster and whether that type uses a numbered dose, and `vaccine_validity_options` stores full value+unit validity presets such as 21 days or 12 months. Each vaccination stores snapshots in `vaccine_name`, `dose_type`, `dose_number`, `validity_value`, and `validity_unit` for display/history; numbered doses are rendered from `dose_number` as labels such as `2ª dose`. Due-date equivalence remains rooted in `vaccine_normalized_name`: when a new application of the same vaccine name is saved for a pet, older active applications are marked with `validity_ignored_at` so only the latest contributes to alerts and analytics.

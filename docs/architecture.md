...
- Preventive records use editable name catalogs in `preventive_catalog_items` with separate `kind` values for vaccines and dewormers. Vaccine applications store free-text `dose` plus `validity_value` and `validity_unit` (`days`, `months`, or `years`), matching the deworming workflow. Optional presets live in `preventive_protocols`, `preventive_protocol_items`, and `preventive_protocol_doses`; selecting a protocol dose fills dose, validity, and observation while keeping the application row as a historical snapshot. Vaccine due-date equivalence remains rooted in `vaccine_normalized_name`: when a new application of the same vaccine name is saved for a pet, older active applications are marked with `validity_ignored_at` so only the latest contributes to alerts and analytics.

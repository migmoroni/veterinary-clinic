# Plano Do Server Open

Atualizado em 2026-08-04.

## Objetivo

`server-open` e o servidor aberto do ecossistema. Ele deve prover dados
...
Os dados fonte publicos ficam no `server-open`, nao dentro dos apps.
...
O `server-open` deve gerar e publicar artefatos como:
...
O app deve conseguir perguntar ao `server-open`:
...
O `server-open` tambem pode ser o ponto publico de distribuicao dos apps:
...
Rails entra como base do `server-open` para:
...
`server-open`, usando servidor local, fixture publicada ou snapshot gerado por
tarefa do proprio `server-open`.
...
2. Migrar dados fonte publicos para o `server-open`.
...
- `server-open` gera e publica `system`, `system_media` e `CAS/system`.
...
- Nenhuma regra SaaS fechada entra no `server-open`.

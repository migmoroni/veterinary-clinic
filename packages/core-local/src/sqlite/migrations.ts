...
import { MANUFACTURER_TYPES, stringifyManufacturerCatalogExtension, stringifyManufacturerType } from '@vet/types/domain/manufacturer/catalog.js';
...
import { ACTIVE_INGREDIENT_TYPES, stringifyActiveIngredientCatalogExtension, stringifyActiveIngredientType } from '@vet/types/domain/active-ingredient/catalog.js';
import { defaultConditionCatalogItems, type DefaultConditionCatalogImage } from '@vet/types/domain/condition/default-catalog.js';
import { CONDITION_TYPES, stringifyConditionCatalogExtension, stringifyConditionType } from '@vet/types/domain/condition/catalog.js';
...
async function createSystemIndexes(database: Database): Promise<void> {
	await database.execute('CREATE INDEX IF NOT EXISTS idx_image_collections_entity ON image_collections(entity_type, entity_id)');
	await database.execute('CREATE INDEX IF NOT EXISTS idx_image_collection_items_collection_id ON image_collection_items(collection_id, sort_order, id)');
...
async function removeSystemDataFromClientDatabase(database: Database): Promise<void> {
...
async function refreshOutdatedSystemCatalogSchema(database: Database): Promise<void> {

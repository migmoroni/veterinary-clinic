...
const vaccinePresetIds = new Map(db.prepare('SELECT id, normalized_name FROM vaccine_presets').all().map((preset) => [normalizeVaccineName(preset.normalized_name), preset.id]));
const ownersCache = new Map();
const canineBreedAliases = [
...
const insertContactFromSource = (report, ownerId, kind, rawValue, sortOrder) => {
    ...
    if (result.changes > 0)
        report.ownerContactsCreated += 1;
    else
        report.ownerContactsReused += 1;
};
...
const skippedReason = (ownerName, petName) => {
    if (!ownerName && !petName)
        return 'sem tutor e sem nome do animal';
    if (!ownerName)
        return 'sem tutor';
    return 'sem nome do animal';
};
const trackPetDuplicateCandidate = (report, row, ownerName, petName) => {
...
const printImportReport = (report) => {
    const skippedByReason = report.skippedRows.reduce((totals, row) => {
    ...
    if (report.skippedRows.length > 10)
    ...
    if (duplicateCandidates.length > 10)
        console.log(`  - ... mais ${duplicateCandidates.length - 10} grupos possíveis`);
...
const printDatabaseReport = () => {
    const vaccinationsWithoutPreset = db.prepare('SELECT COUNT(*) AS total FROM pet_vaccinations WHERE vaccine_preset_id IS NULL').get().total;
...
const getSex = (row) => {
    if (isTruthy(row['MACHO']))
        return 'M';
    if (isTruthy(row['FEMEA']))
        return 'F';
    return null;
};
...
const trackDiscardedVaccinationDate = (report, rawDate, appliedAt) => {
    report.vaccinationDatesDiscarded += 1;
    if (report.vaccinationDateDiscardSamples.length < 10)
        report.vaccinationDateDiscardSamples.push(`${rawDate.trim()} -> ${appliedAt}`);
};
const hasPositiveVaccinationSignal = (normalizedLine) => {
    return /\b(?:vac|vacinad[ao]s?|revacinad[ao]s?|aplicad[ao]s?|imunizad[ao]s?)\b/.test(normalizedLine);
};
const hasNegativeVaccinationSignal = (normalizedLine) => {
...
const extractVaccinesFromLine = (line) => {
    const normalizedLine = normalizeText(line);
    if (!hasPositiveVaccinationSignal(normalizedLine) || hasNegativeVaccinationSignal(normalizedLine))
        return [];
    const vaccines = [];
    for (const matcher of vaccineMatchers) {
        if (matcher.pattern.test(normalizedLine))
            vaccines.push(matcher.name);
    }
    return vaccines;
};
const extractVaccinationsFromRecord = (description, report) => {
    const extracted = [];
...
const detectSpecies = (rawBreed) => {
    const normalized = normalizeText(rawBreed);
    if (['felina', 'felino', 'felin', 'gato', 'gata', 'siames', 'persa'].some((alias) => normalized.includes(alias)))
        return 'feline';
    return 'canine';
};
const cleanBreedText = (rawBreed) => {
    return (rawBreed ?? '')
...
const resolveBreed = (species, rawBreed) => {
...
const getTaxonomy = (row) => {
    const rawBreed = row['RAÇA'];
    const species = detectSpecies(rawBreed);
    return { species, breed: resolveBreed(species, rawBreed) };
};
const isAtLeastAsRecentVaccination = (candidate, current) => {
    if (candidate.appliedAt !== current.appliedAt)
        return candidate.appliedAt > current.appliedAt;
    return candidate.id > current.id;
};
const ignoreVaccinationValidity = (report, id) => {
...
const processarMigracao = () => {
    ...
    const migrateRows = db.transaction((records) => {
        ...
        const latestVaccinationByPetAndPreset = new Map();
        for (const row of records) {
            ...
            if (description) {
                ...
                if (period.admittedAt)
                    report.medicalRecordPeriodsDerived += 1;
                else
                    report.medicalRecordPeriodsMissing += 1;
                if (period.dischargedAt)
                    report.medicalRecordPeriodsWithDischarge += 1;
                for (const vaccination of extractVaccinationsFromRecord(description, report)) {
                    ...
                    if (!vaccinePresetId)
                        throw new Error(`Preset de vacina não encontrado: ${vaccination.vaccine}`);
                    ...
                    if (!currentLatest || isAtLeastAsRecentVaccination(vaccinationReference, currentLatest)) {
                        if (currentLatest)
                            ignoreVaccinationValidity(report, currentLatest.id);
